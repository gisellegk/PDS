...
CHECK_KEY0:
	...
	movia	r10, SW_BASE			# base address of SW slider switches parallel port
	ldwio	r12, 0(r10)				# load a new pattern from the SW switches
	movia	r10, PATTERN			# set up a pointer to the pattern variable
	stw		r12, 0(r10)				# store the new pattern to the global variable

CHECK_KEY1:
	andi	r13, r11, 0b0010		# check KEY1
	beq		r13, zero, END_PUSHBUTTON_ISR
	movia	r10, SHIFT_DIR			# set up a pointer to the shift direction variable
	ldw		r12, 0(r10)				# load the shift direction variable
	xori	r12, r12, 1				# toggle the directon
	stw		r12, 0(r10)				# store the new direction to the global variable
